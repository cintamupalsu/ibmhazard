...
// 2 Disaster Name
// 3 iteration (i.e. 60 = 60 min, 120 =120 min ...)
// 4 Middle Point 0- Highest lowest 1 - average (Landslide must filled with 0, tsunami with 0)
// 5 Anchor Name
class Geodesic{
	...
	public static void main(String args[]) {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(args[1]))){
		    ...
		    while ((line = br.readLine()) != null) {
		        ...
		        zl[zl.length-1].lon=Double.parseDouble(d1Line[2]);
						int x = Integer.parseInt(d1Line[3]);
						int y = Integer.parseInt(d1Line[4]);
						String xs = Integer.toString(x);
						String ys = Integer.toString(y);
		        zl[zl.length-1].name=d1Line[0];
						System.out.println(d1Line[0]);
					}else{
						countZoneLine+=1;
					}
		    }
		...
		for(int i=0;i<fh.ncols;i++) {
			for(int j=0;j<fh.nrows;j++) {
				...
				if(cubic[i][j].value<=stg[2] && cubic[i][j].value>stg[1]) {
					cubic[i][j].stage=2;
				}
				if(cubic[i][j].value>stg[2]) {
					cubic[i][j].stage=3;
				}								
			}
		}
		...
		try (BufferedWriter bw = new BufferedWriter(new FileWriter("./result/seeds.rb"))){
			bw.write(args[2]+";"+args[5]+";"+args[3]+"\n"); // hazard name and anchor name
				
			for(int m=0; m<poly.length; m++) {
				int zone_id=poly[m].zoneID+1;
				String placeName = zl[zone_id-1].name;
				String inside = "";
				...
				for(int m1=0; m1<4; m1++) {
					rectangle+=poly[m].lat[m1]+";"+poly[m].lon[m1]+";";
				}
				bw.write(placeName+";"+poly[m].level+";"+rectangle+inside+"\n");				
			}
			bw.close();
		}catch(IOException e) {
